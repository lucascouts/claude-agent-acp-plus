...
export class ClaudeAcpAgent {
  ...
  canUseTool(sessionId: string): CanUseTool {
    return async (toolName, toolInput, { signal, suggestions, toolUseID, agentID }) => {
      ...
      if (toolName === "AskUserQuestion" && askUserQuestionFallbackEnabled(process.env)) {
        ...
        this.logger.error(
          "AskUserQuestion: routing via permission fallback (client lacks elicitation.form).",
        );
        return handleAskUserQuestionViaPermission(
          toolInput,
  ...
  private async createSession(
    ...
    const disallowedTools =
      elicitationSupport.form || askUserQuestionFallbackEnabled(process.env)
        ? []
        : ["AskUserQuestion"];
    ...
    const contextWindowSize =
      ...
      inferContextWindowFromModel(
        ...
        catalogModelInfo?.resolvedModel,
        catalogModelInfo?.displayName,
        catalogModelInfo?.description,
      ) ??
      DEFAULT_CONTEXT_WINDOW;
