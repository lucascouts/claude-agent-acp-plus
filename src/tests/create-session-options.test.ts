...
describe("createSession options merging", () => {
  ...
  let originalAskFallback: string | undefined;
  ...
  // The AskUserQuestion fallback gate defaults ON, which would expose the tool
  // for clients without elicitation.form. Pin the suite to the gate-OFF
  // (upstream-parity) baseline so the disallowedTools-merging tests keep
  // asserting the ACP-internal disallow; the elicitation block re-enables the
  // gate where it exercises the new behavior. Snapshot/restore so it never leaks.
  beforeEach(() => {
    originalAskFallback = process.env.ACP_ASKUSERQUESTION_FALLBACK;
    process.env.ACP_ASKUSERQUESTION_FALLBACK = "0";
  });

  afterEach(() => {
    if (originalAskFallback !== undefined) {
      process.env.ACP_ASKUSERQUESTION_FALLBACK = originalAskFallback;
    } else {
      delete process.env.ACP_ASKUSERQUESTION_FALLBACK;
    }
  });
  ...
  describe("elicitation", () => {
    // These cases pin the fallback-gate matrix, so start each from the gate-ON
    // default (var unset); the outer suite baseline otherwise sets it to "0",
    // and the outer afterEach restores the real ambient value afterwards.
    beforeEach(() => {
      delete process.env.ACP_ASKUSERQUESTION_FALLBACK;
    });

    it("keeps AskUserQuestion enabled via the fallback gate and omits callbacks without elicitation capability", async () => {
    ...
    it.each(["0", "false"])(
      "disables AskUserQuestion without elicitation capability when the fallback gate is off (=%s)",
      async (value) => {
        process.env.ACP_ASKUSERQUESTION_FALLBACK = value;
        await agent.initialize({ protocolVersion: 1, clientCapabilities: {} });
        await agent.newSession({ cwd: process.cwd(), mcpServers: [] });
    ...
    it("wires callbacks for url-only elicitation and keeps AskUserQuestion enabled via the fallback gate", async () => {
      await agent.initialize({
        protocolVersion: 1,
