#!/usr/bin/env bats
#
# The waiver path of scripts/ci-attest.sh (R2.1 to R2.5, R5.2).
#
# `act` is not the GitHub runner. Two gaps are known: harden-runner has no
# equivalent outside GitHub's infrastructure, and setup-node's npm cache depends
# on GitHub's hosted cache. Neither is a defect in the code under test, and
# neither may quietly colour a result — so an environmental failure publishes
# nothing at all unless the maintainer waives it with a stated reason, and that
# reason travels into the status description so the PR records it.
#
# The waiver flag is part of the contract these tests pin: `--waive "<reason>"`.
#
# `act` and `gh` are replaced by doubles first on PATH; no container is started
# and no network call is made (R5.3).
...
@test "a genuine job failure publishes a failure status and exits non-zero" {
  require_script
  export ACT_FAIL_JOB=build

  run attest
  [ "$status" -ne 0 ]
  status_for 'Build' | grep -qi 'failure'
  ! status_for 'Build' | grep -qi 'success'
}

@test "an environmental failure publishes nothing without a waiver" {
  require_script
  export ACT_ENV_FAIL_JOB=gitleaks

  run attest
  [ "$status" -ne 0 ]
  # The check stays absent rather than green — and the report names the step the
  # local runner could not reproduce.
  [ -z "$(status_for 'Secret scan (gitleaks)')" ]
  [[ "$output" == *"Harden the runner"* || "$output" == *"harden-runner"* ]]
}

@test "an environmental failure is publishable under a waiver carrying the reason" {
  require_script
  export ACT_ENV_FAIL_JOB=gitleaks

  run attest --waive "harden-runner has no equivalent outside GitHub"
  [ -n "$(status_for 'Secret scan (gitleaks)')" ]
  status_for 'Secret scan (gitleaks)' | grep -qF 'harden-runner has no equivalent outside GitHub'
}

@test "refuses a waiver whose reason is empty" {
  require_script
  export ACT_ENV_FAIL_JOB=gitleaks
...
@test "reports the steps the local runner did not reproduce, waiver or not" {
  require_script
  export ACT_ENV_FAIL_JOB=gitleaks

  run attest --waive "accepted: no local equivalent"
  [[ "$output" == *"Harden the runner"* || "$output" == *"harden-runner"* ]]
}
