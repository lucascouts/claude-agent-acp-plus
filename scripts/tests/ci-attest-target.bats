#!/usr/bin/env bats
#
# How scripts/ci-attest.sh works out what it is attesting (R1.4, R1.5, R4.3).
#
# The script takes no required argument: it derives the repository from the git
# remote — never a hard-coded owner, because the same file lives in two
# repositories — finds the open release PR and resolves its head SHA. When a
# prerequisite is missing it says which one and publishes nothing.
#
# `gh`, `act`, `docker` and `podman` are replaced by doubles first on PATH that
# record their arguments. No network call is made and no container is started
# (R5.3).
...
@test "resolves the repository from the git remote, hard-coding no owner" {
  require_script

  run attest
  [ "$status" -eq 0 ]
  grep -q 'acme/widget' "$GH_LOG"
  ! grep -qiE 'lucascouts|claude-agent-acp' "$GH_LOG"
}

@test "fails with an actionable message when no open release PR exists" {
  require_script
  printf '%s\n' '[]' >"$FIX/pr-list.json"
  printf '%s\n' '{}' >"$FIX/pr-view.json"
...
@test "fails naming the container runtime when it is unavailable" {
  require_script
  export ACT_RUNTIME_DOWN=1

  run attest
  [ "$status" -ne 0 ]
  [[ "$output" == *"docker"* || "$output" == *"Docker"* || "$output" == *"container runtime"* ]]
  [ -z "$(published_statuses)" ]
}

@test "takes the head SHA from the release PR rather than from an argument" {
  require_script
