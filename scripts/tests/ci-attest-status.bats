#!/usr/bin/env bats
#
# What scripts/ci-attest.sh publishes to the release PR (R1.2, R1.3, R3.6).
#
# The context string is the whole point: the preflight looks for these four
# names, and a status published under any other spelling is invisible to it. The
# status goes to the pull request's current head commit, so that an attestation
# never survives release-please rewriting the PR.
#
# `gh` is replaced by a double first on PATH that records every call instead of
# sending it (R5.3).
...
@test "publishes one status per job, with the exact context strings" {
  require_script
  ...
  [ -n "$(status_for 'Dependency scan (OSV-Scanner)')" ]
  [ -n "$(status_for 'npm audit')" ]
}

@test "publishes exactly four statuses" {
  require_script

  run attest
  [ "$status" -eq 0 ]
  [ "$(published_statuses | grep -c 'statuses/')" -eq 4 ]
}

@test "publishes to the pull request's head commit" {
  require_script

  run attest
  [ "$status" -eq 0 ]
  [ "$(published_statuses | grep -c "statuses/$HEAD_SHA")" -eq 4 ]
}

@test "publishes success for a job that passed" {
  require_script

  run attest
  [ "$status" -eq 0 ]
  status_for 'Build' | grep -qi 'success'
  status_for 'npm audit' | grep -qi 'success'
}

@test "publishes nothing before the jobs have run" {
  require_script
  export ACT_RUNTIME_DOWN=1
